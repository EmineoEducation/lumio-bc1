...
function LivrableApp() {
  ...
  const [activeTab, setActiveTab] = React.useState('C.1');
  const rowRefs = React.useRef({});
  const scrollRef = React.useRef(null);

  const scrollToTab = (code) => {
    setActiveTab(code);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#f9f8f5', fontFamily: 'var(--font-sans)', overflow: 'hidden' }}>
      ...
      {/* Header */}
      <div style={{ padding: '12px 24px 0', borderBottom: '1px solid var(--rule)', background: 'white', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 10 }}>
          <div>
            <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--ink)', marginBottom: 2 }}>Livrable — BC1 · Lumio Health</div>
            <div style={{ fontSize: 11, color: 'var(--ink-faint)', fontFamily: 'var(--font-mono)', letterSpacing: '0.06em' }}>RNCP 38504 · 30 septembre 2026 · CODIR 09h00</div>
          </div>
          <div style={{ textAlign: 'right', flexShrink: 0 }}>
            <div style={{ fontSize: 20, fontWeight: 700, color: globalWords >= GLOBAL_MIN ? '#1a6641' : 'var(--ink-mute)', fontFamily: 'var(--font-mono)', lineHeight: 1 }}>{globalWords}<span style={{ fontSize: 11, fontWeight: 400, color: 'var(--ink-faint)', marginLeft: 4 }}>/500</span></div>
            <div style={{ fontSize: 10, color: 'var(--ink-faint)', letterSpacing: '0.08em', textTransform: 'uppercase', marginTop: 2 }}>MOTS GLOBAUX</div>
          </div>
        </div>
        {/* Onglets C.1–C.6 */}
            ...
            return (
              <button
                ...
                style={{
                  ...
                  cursor: 'pointer',
                  background: isActive ? 'var(--accent)' : ok ? 'rgba(26,102,65,0.10)' : 'rgba(20,24,36,0.06)',
                  color: isActive ? 'white' : ok ? '#1a6641' : 'var(--ink-mute)',
                  transition: 'all .12s',
      ...
      {/* Tableau scrollable */}
      <div ref={scrollRef} style={{ flex: 1, overflowY: 'auto', padding: '16px 20px 16px', minHeight: 0 }}>
        ...
        {COMPETENCES.map((c, i) => {
          ...
          return (
            <div
              key={c.code}
              ref={el => { if (el) rowRefs.current[c.code] = el; }}
              style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: 0, borderBottom: isLast ? 'none' : '1px solid var(--rule)' }}
            >
              ...
              <div style={{ background: '#f4f2ee', padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                <div style={{ marginTop: 4, padding: '8px 10px', background: 'rgba(196,66,15,0.06)', borderRadius: 4, border: '1px solid rgba(196,66,15,0.12)' }}>
                  <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--accent)', fontFamily: 'var(--font-mono)', letterSpacing: '0.08em', marginBottom: 4 }}>CONSEIL</div>
                  <div style={{ fontSize: 11, color: 'var(--ink-soft)', lineHeight: 1.5 }}>
                    {c.code === 'C.1' && 'Cite tes sources explicitement (Les Échos, HBR, verbatims Camille…) et classe-les par catégorie.'}
                    {c.code === 'C.2' && 'Pour chaque signal, nomme l\'impact concret sur Lumio — pas juste "risque" ou "opportunité" en abstrait.'}
                    {c.code === 'C.3' && 'Sois explicite sur les contradictions : Sonia dit 230 contacts, Théo dit 180. Qui a raison et pourquoi ?'}
                    {c.code === 'C.4' && 'Un diagnostic, c\'est une thèse, pas une liste. Commence par ta conclusion, prouve-la ensuite.'}
                    {c.code === 'C.5' && 'L\'écart B2B/B2C est réel mais pas le seul. La certif MDR crée un écart entre ce que Lumio promet et ce qu\'elle peut tenir.'}
                    {c.code === 'C.6' && 'Chaque élément de ta plateforme doit être défendable face à Théo : qu\'est-ce que tu peux promettre sans la certif ?'}
                  </div>
                </div>
      ...
      {/* Footer — soumettre */}
      <div style={{ padding: '10px 24px', borderTop: '1px solid var(--rule)', background: 'white', display: 'flex', alignItems: 'center', gap: 16, flexShrink: 0 }}>
        <div style={{ flex: 1 }}>
          {missingMin.length > 0 ? (
